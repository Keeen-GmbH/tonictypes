 ...
 *
 * Slightly changed to add TYPO3 compatibility
 *
...
trait jQueryQueryBuilderFunctions
{
     ...
     * @throws \Exception
     */
    protected function convertArrayToFlatValue(string $field, $value)
    {
        if (count($value) !== 1) {
     ...
     * @throws \Exception
     */
    protected function makeQueryWhenArray(QueryBuilder $queryBuilder, stdClass $rule, array $sqlOperator, array $value, $condition): QueryBuilder
    {
     ...
     * @throws \Exception
     */
    protected function makeQueryWhenNull(QueryBuilder $queryBuilder, stdClass $rule, array $sqlOperator, string $condition): QueryBuilder
    {
     ...
     * @return QueryBuilder
     */
    private function makeArrayQueryIn(QueryBuilder $queryBuilder, stdClass $rule, $operator, array $value, $condition): QueryBuilder
    {
        if ($operator == 'NOT IN') {
            if ($condition === 'and') {
     ...
     * @see makeQueryWhenArray
     */
    private function makeArrayQueryBetween(QueryBuilder $queryBuilder, stdClass $rule, array $value, string $condition): QueryBuilder
    {
        if (count($value) !== 2) {
