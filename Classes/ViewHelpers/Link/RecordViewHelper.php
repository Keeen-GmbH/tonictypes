...
namespace K3n\Tonictypes\ViewHelpers\Link;
...
class RecordViewHelper extends AbstractTagBasedViewHelper
{
    ...
    public function render(): string
    {
        ...
        throw new \RuntimeException(
            'The rendering context of ViewHelper dv:link.record is missing a valid request object.',
            1690365240
        );
    }
    ...
    protected function renderFrontendLinkWithCoreContext(ServerRequestInterface $request): string
    {
        ...
        $record = $this->arguments['record'] ?? null; // DV-specific
        ...
        if (!$allExtbaseArgumentsAreSet) {
            throw new \RuntimeException(
                'ViewHelper dv:link.record needs either all extbase arguments set'
                . ' ("extensionName", "pluginName", "controller", "action", "record")'
    ...
    protected function renderWithExtbaseContext(ExtbaseRequestInterface $request): string
    {
        ...
        // DV-specific
        if (is_numeric($record)) {
            $parameters['record'] = (int)$record;
        }

        // DV-specific
        if ($record instanceof AbstractRecordModel) {
            $parameters['record'] = $record->getUid();
        }
